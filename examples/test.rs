...
fn main() -> Result<(), eframe::Error> {
    ...
    eframe::run_native("Working Draggable Nodes", options, Box::new(|cc| Box::new(App::new(cc))))
}
...
impl App {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        for i in 0..5 {
            ...
            let position = egui::Pos2::new(100.0 + i as f32 * 150.0, 100.0 + (i % 2) as f32 * 100.0);
            let title = format!("Node {}", i);
            nodes.insert(id, Node { position, title, selected: false });
        }
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if self.selection_rect.is_none() && pointer.any_pressed() && 
               !ctx.is_using_pointer() && available_rect.contains(pointer_pos) {
            ...
            egui::Window::new("Debug Info")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.label(format!("Selection rect: {:?}", self.selection_rect.is_some()));
                    
                    ui.separator();
                    for (id, node) in &self.nodes {
                        ui.label(format!("Node {:?}: pos={:?}, selected={}", 
